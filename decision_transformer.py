...
class DecisionTransformer(nn.Module):
    ...
    def __init__(self):
        ...
        max_time_length = max(FORECAST_LENGHT, MAX_LEAD_TIME) #hago esto para que el embedding de tiempo tenga suficiente capacidad para almacenar los valores de tiempo de todos los datos (demanda y stock en tránsito)
        #self.maxSeqLength hiperparametro que determina cuantos pasos de historia se tienen en cuenta para la predicción
        self.projectTimeData = nn.Embedding(max_time_length, self.embeddingDim) 
    ...
    def initModel(self, td): 
        batchSize = 2  #change this constant to the batch size of the data     
        ...
        tdNew["inTransitStock"] = torch.zeros(batchSize, 14, dtype=torch.int64) #initially the in transit stock is 0
        ...
        tdNew["benefit"] = torch.zeros(batchSize, dtype=torch.int64)
    ...
    # Debería ser [batch_size, FORECAST_LENGTH, embeddingDim]    
        ...
        stackedInputs = (
                torch.stack((td["returnsToGoEmbedding"], td["statesEmbedding"],
                             torch.cat((td["actionsEmbedding"],
                                        torch.zeros(batchSize, 1, self.embeddingDim, device=self.device)), dim=1)),
                            dim=1)
                .permute(0, 2, 1, 3)
        ...
        orderQuantity = self.relu(orderQuantity)  # Asegura que la cantidad a ordenar sea no negativa
        ...
        actionEmbedding = self.embeddingAction(orderQuantity)  # Necesitas añadir esta capa en __init__
        ...
        td["onHandLevel"] = td["onHandLevel"] + td["inTransitStock"][..., 0]
        ...
        td["onHandLevel"] = max(0, td["onHandLevel"] - td["forecast"][..., 0]) #no se si puede ser 0 mirAR BIEN COMO SE ACTUALIZA FORECAST

        holdingCost = td["holdingCost"] * td["onHandLevel"]
        if orderQuantity > 0:
            orderingCost = td["orderingCost"]
        else:
            orderingCost = 0
        ...
        currentTimeStep = td["currentTimestep"].item()
        td["benefit"][currentTimeStep] = income - holdingCost - stockOutPenalty - orderingCost
        returnToGo = td["returnsToGo"]

        if currentTimeStep >= RETURN_TO_GO_WINDOW:
            returnToGo = (returnToGo*RETURN_TO_GO_WINDOW - td["benefit"][currentTimeStep] + td["benefit"][currentTimeStep-RETURN_TO_GO_WINDOW])/RETURN_TO_GO_WINDOW
        
        td["returnsToGo"] = returnToGo
    ...
    def addSequenceData(self, td, tensor, data):
        if tensor.size(1) == FORECAST_LENGHT: #self.maxSeqLength
            return torch.cat((tensor[:, 1:, :], data + self.projectTimeData(td["currentTimestep"])), dim=1)
        return torch.cat((tensor, data + self.projectTimeData(td["currentTimestep"])), dim=1)
...
if __name__ == "__main__":
    ...
    td = TensorDict({
        ...
        'leadTime': torch.tensor([[15]] * batch_size),  
        ...
        'inTransitStock': torch.zeros(batch_size, 14),  # Sin pedidos en tránsito inicialmente
